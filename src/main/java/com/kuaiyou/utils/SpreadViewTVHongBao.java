...
public class SpreadViewTVHongBao extends SpreadView implements View.OnTouchListener {
    ...
    public SpreadViewTVHongBao(Context context) {
        super(context);
        int[] screenSize = AdViewUtils.getWidthAndHeight(context.getApplicationContext(), false, true);
...
//        this.setBackgroundColor(Color.WHITE);
    ...
    public int getAdHeight(int hasLogo) {
        return screenHeight - (hasLogo * (screenHeight / 5));
    }
    ...
    public void loadAdLayout(int width, int height, int adType, int layoutType, int deformation, int hasLogo, MRAIDNativeFeatureListener nativeFeatureListener, MRAIDViewListener mraidViewListener) {
            ...
            default:
                ...
                if (layoutType == ConstantValues.EXTRA3 || hasLogo == 0) {
                    ...
                    if (deformation == ConstantValues.SCALE_INCLUDEHTML)
                        sizeMap.put(ConstantValues.INSTLHEIGHT, screenHeight * 4 / 5);
        ...
        addView(instlView);
        instlView.removeView(instlView.findViewById(ConstantValues.BEHAVICONID));
                ...
                switch (child.getId()) {
                        ...
                        switch (layoutType) {
                            ...
                            case ConstantValues.EXTRA1:
                                if (hasLogo == ConstantValues.HASLOGO)
                                    child.layout(0, screenHeight - (screenHeight / 5), screenWidth, screenHeight);
                                else
                        ...
                        switch (deformation) {
                            ...
                            case ConstantValues.SCALE_INCLUDEHTML:
                                switch (layoutType) {
                                    case ConstantValues.CENTER:
                                        if (!isHtml)
                                            child.layout(0, 0, sWidth, (screenHeight - (screenHeight / 5 * hasLogo)));
                                        else
                                            child.layout(0, (screenHeight - (screenHeight / 5 * hasLogo)) / 2 - sHeight / 2, sWidth, (screenHeight - (screenHeight / 5 * hasLogo)) / 2 + sHeight / 2);
                                        break;
                                    case ConstantValues.BOTTOM:
                                        if (!isHtml)
                                            child.layout(0, 0, sWidth, (screenHeight - (screenHeight / 5 * hasLogo)));
                                        else
                                            child.layout(0, (screenHeight - (screenHeight / 5 * hasLogo)) - sHeight, sWidth, screenHeight - (screenHeight / 5 * hasLogo));
                                        break;
                                    case ConstantValues.TOP:
                                    case ConstantValues.ALL_CENTER:
                                    case ConstantValues.EXTRA3:
                    ...
                    case ConstantValues.ADICONID:
                        if (null != findViewById(ConstantValues.SPREADLOGOIMAGEID) && 0 != findViewById(ConstantValues.SPREADLOGOIMAGEID).getHeight())
                            ...
                            else if (sHeight > screenHeight - (screenHeight / 5))
                                child.layout(0, screenHeight - screenHeight / 5 - screenWidth / 25, screenWidth / 8, screenHeight - screenHeight / 5);
                            else
                    ...
                    case ConstantValues.ADLOGOID:
                        if (null != findViewById(ConstantValues.SPREADLOGOIMAGEID) && 0 != findViewById(ConstantValues.SPREADLOGOIMAGEID).getHeight()) {
                            if (sHeight == screenHeight)
                                ...
                                child.layout(screenWidth - (screenWidth / 8), screenHeight - screenHeight / 5 - screenWidth / 25, screenWidth, screenHeight - screenHeight / 5);
                            else
                    ...
                    case ConstantValues.SPREADTEXTID:
                        if (hasLogo == ConstantValues.HASLOGO) {
                            if (null != ((RelativeLayout) child.getParent()).findViewById(ConstantValues.SPREADLOGOIMAGEID)) {
                                if (sHeight > screenHeight - (screenHeight / 5))
                                    child.layout(0, (screenHeight - screenWidth / 4 - screenHeight / 5), screenWidth * 3 / 4, (screenHeight - (screenHeight / 5)));
                                else
                    ...
                    case ConstantValues.BEHAVICONID:
                        if (hasLogo == ConstantValues.HASLOGO) {
                            if (null != ((RelativeLayout) child.getParent()).findViewById(ConstantValues.SPREADLOGOIMAGEID)) {
                                if (sHeight > screenHeight - (screenHeight / 4))
                                    child.layout((screenWidth * 3 / 4), (screenHeight - screenWidth / 4 - screenHeight / 5), screenWidth, (screenHeight - screenHeight / 5));
                                else
...
//                if (Math.abs(event.getX() - downX) < padding * padding / 2 && Math.abs(event.getY() - downY) < padding * padding / 2 && event.getEventTime() - event.getDownTime() < 1000) {
                    if (closeRect.contains((int) event.getRawX(), (int) event.getRawY())) {
                        ...
                        if (null != kyViewListener)
                            kyViewListener.onViewClicked(event, null, null, downX, downY);
